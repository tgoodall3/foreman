...
export default async function OwnerToday() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 mx-auto w-full max-w-6xl space-y-6">
      {/* Header */}
          ...
          {todayJobs.length === 0 ? (
            ...
            <div className="divide-y divide-gray-100">
              {todayJobs.map((job: any) => {
                ...
                return (
                  <div key={job.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:gap-4">
        ...
        <div className="space-y-4">
          ...
          <section className="bg-white border border-gray-200 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2 p-3">
              {[{href:"/owner/jobs/new",label:"New Job",icon:"🛠"}, {href:"/owner/estimates/new",label:"New Estimate",icon:"📝"}, {href:"/owner/invoices/new",label:"New Invoice",icon:"💵"}, {href:"/owner/schedule",label:"Schedule",icon:"📆"}].map((a) => (
...
function ActionCard({ icon, title, count, body, href, cta, tone = "gray" }: {
  icon: string; title: string; count: number; body: string; href: string; cta: string; tone?: "gray" | "red";
}) {
  const border = tone === "red" ? "border-red-200" : "border-gray-200";
  const bg     = tone === "red" ? "bg-red-50/60" : "bg-white";
  const text   = tone === "red" ? "text-red-700" : "text-forge";
  return (
    <div className={`rounded-2xl border ${border} ${bg} shadow-sm p-4 flex items-center justify-between gap-3`}>
      <div className="min-w-0">
        <div className="flex items-center gap-2 text-sm font-700 text-forge">
          <span aria-hidden>{icon}</span>
          <span>{title}</span>
        </div>
        <p className={`text-xs mt-1 ${tone === "red" ? "text-red-700" : "text-mist"}`}>{body}</p>
      </div>
      <div className="flex flex-col items-end gap-2">
        <span className={`px-2 py-1 rounded-lg text-xs font-700 ${text} bg-gray-100`}>{count}</span>
        <Link href={href} className="text-xs font-700 text-forge bg-amber/80 hover:bg-amber text-forge px-3 py-1.5 rounded-lg transition-colors shadow-sm">
          {cta} →
        </Link>
      </div>
    </div>
