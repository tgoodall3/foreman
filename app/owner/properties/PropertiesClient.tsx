...
export default function PropertiesClient({ propertyManagers: initial, tenantId, appUrl }: any) {
  ...
  const handleAddPm = async (e: React.FormEvent) => {
    ...
    setView("list");
    addToast(data.reused ? t("properties.pmReused") : t("properties.pmAdded"), "success");
    setSubmitting(false);
  };
  ...
  const handleAddProperty = async (e: React.FormEvent) => {
    ...
    setView("list");
    addToast(t("properties.propertyAdded"), "success");
    setSubmitting(false);
  };
  ...
  const handleToggleAccess = async (pm: any) => {
    ...
    addToast(data.is_active ? t("properties.accessRestored", { name: pm.full_name }) : t("properties.accessRevokedMsg", { name: pm.full_name }), data.is_active ? "success" : "error");
  };
  ...
  const handleSendPortalLink = async (pm: any) => {
    ...
    addToast(t("properties.portalEmailed", { email: pm.email }), "success");
  };

  const handleDeletePm = async (pm: any) => {
    const confirmed = window.confirm(t("properties.deleteConfirm", { name: pm.full_name }));
    if (!confirmed) return;
    ...
    addToast(
      t("properties.deleteSuccess", { name: pm.full_name, properties: counts.properties ?? 0, workOrders: counts.workOrders ?? 0, invoices: counts.invoices ?? 0, estimates: counts.estimates ?? 0 }),
      "success"
    );
  };
    ...
    <div className="page-shell page-shell-standard">
      <div className="page-header">
        <div className="page-header-copy">
          ...
          <p className="page-subtitle">{t("properties.pmCount", { count: pms.length })}</p>
        </div>
      ...
      {view === "add-pm" && (
        <div className="surface-card mb-6 p-4 sm:p-5">
          <h2 className="font-display font-700 text-lg text-forge mb-4">{t("properties.addPMTitle")}</h2>
          <form onSubmit={handleAddPm} noValidate className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.fullName")} *</label>
              <input value={pmName} onChange={(e) => setPmName(e.target.value)} required className={inp} placeholder="Jane Smith" /></div>
            <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.email")} *</label>
            ...
            <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.phone")}</label>
              <input type="tel" value={pmPhone} onChange={(e) => setPmPhone(e.target.value)} className={inp} placeholder="(555) 000-0000" /></div>
            <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.company")}</label>
            ...
            <div className="col-span-1 sm:col-span-2 flex flex-col sm:flex-row gap-3">
              <button type="button" onClick={() => setView("list")} className="action-button-secondary order-2 w-full sm:order-1 sm:w-auto">{t("common.cancel")}</button>
      ...
      {view === "add-property" && selectedPm && (
        <div className="surface-card mb-6 p-4 sm:p-5">
          <h2 className="font-display font-700 text-lg text-forge mb-1">{t("properties.addProperty")}</h2>
          <p className="text-sm text-mist mb-4">{t("properties.forPM", { name: selectedPm.full_name })}</p>
          <form onSubmit={handleAddProperty} noValidate className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="col-span-2"><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.propertyName")} *</label>
              <input value={propName} onChange={(e) => setPropName(e.target.value)} required className={inp} placeholder={t("properties.propertyNamePlaceholder")} /></div>
            <div className="col-span-2"><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.streetAddress")} *</label>
            ...
            <div className="col-span-2 grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.city")} *</label>
                <input value={propCity} onChange={(e) => setPropCity(e.target.value)} required className={inp} placeholder="Indianapolis" /></div>
              <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.state")} *</label>
                <input value={propState} onChange={(e) => setPropState(e.target.value)} required maxLength={2} className={inp} placeholder="IN" /></div>
              <div><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.zip")} *</label>
                <input value={propZip} onChange={(e) => setPropZip(e.target.value)} required className={inp} placeholder="46240" /></div>
            </div>
            <div className="col-span-2"><label className="block text-xs font-600 text-mist uppercase tracking-wider mb-1">{t("properties.notes")}</label>
              <textarea value={propNotes} onChange={(e) => setPropNotes(e.target.value)} rows={2} className={inp + " resize-none"} placeholder={t("properties.notesPlaceholder")} /></div>
            {error && <div className="col-span-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{error}</div>}
            <div className="col-span-1 pt-1 sm:col-span-2 sm:pt-2">
              <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setView("list")} className="action-button-secondary order-2 w-full whitespace-nowrap sm:order-1 sm:w-auto">{t("common.cancel")}</button>
      ...
      {pms.length === 0 ? (
        <div className="surface-empty py-16">
          ...
          <p className="font-display font-700 text-xl text-forge mb-1">{t("properties.noPMs")}</p>
          <p className="text-mist text-sm">{t("properties.noPMsNote")}</p>
        </div>
      ) : (
          ...
          {pms.map((pm: any) => (
            <div key={pm.id} className="surface-card overflow-hidden">
              <div className="border-b border-gray-100 px-4 py-4 sm:px-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {pm.is_active === false && (
                      <span className="text-[10px] font-700 uppercase tracking-wider bg-red-100 text-red-600 px-1.5 py-0.5 rounded">{t("properties.accessRevoked")}</span>
                    )}
                  </div>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="shrink-0 rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-700 uppercase tracking-wider text-steel">
                      {pm.properties?.length ?? 0} {(pm.properties?.length ?? 0) === 1 ? t("properties.propertySingular") : t("properties.propertyPlural")}
                    </div>
                  </div>
                </div>
                <div className="mt-3 grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
                  {pm.is_active !== false && (
                    <>
                      ...
                      >
                        {copiedToken === pm.portal_token ? t("properties.copied") : t("properties.copyPortalLink")}
                      </button>
                      <button
                      ...
                      >
                        {sendingId === pm.id ? t("properties.sending") : t("properties.emailPortalLink")}
                      </button>
                      <button
                      ...
                      >
                        {t("properties.addPropertyButton")}
                      </button>
                    </>
                  )}
                  <button
                  ...
                  >
                    {togglingId === pm.id ? "…" : pm.is_active === false ? t("properties.restoreAccess") : t("properties.revokeAccess")}
                  </button>
                  {pm.is_active === false && (
                    <button
                    ...
                    >
                      {deletingId === pm.id ? t("properties.deleting") : t("properties.deletePM")}
                    </button>
                  )}
                </div>
              </div>
              {pm.properties?.length > 0 ? (
                ...
                <div className="px-5 py-3 text-xs text-mist italic">{t("properties.noProperties")}</div>
              )}
            </div>
          ))}
        </div>
